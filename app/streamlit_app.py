...
if cotation_file and restriction_file:
    ...
    st.write("### 🔍 Vérification des colonnes")

    st.write("Colonnes cotation :", list(cotation.columns))
    st.write("Colonnes restriction :", list(restriction.columns))

    # Colonnes à exclure (cohérent avec ton matching.py)
    cols_to_exclude = ["Poste", "Matricule", "precision", "Engin"]

    # Colonnes communes réellement utilisées
    colonnes_communes = [
        col for col in restriction.columns
        if col in cotation.columns
    ...
    st.write("### ✅ Colonnes communes utilisées pour le matching")
    st.write(f"Nb colonnes communes : {len(colonnes_communes)}")
    ...
    only_restriction = set(restriction.columns) - set(cotation.columns)
    only_cotation = set(cotation.columns) - set(restriction.columns)

    st.write("### ⚠️ Colonnes uniquement dans restriction")
    st.write(list(only_restriction))

    st.write("### ⚠️ Colonnes uniquement dans cotation")
    st.write(list(only_cotation))
